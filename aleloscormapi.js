...
function doLMSSetValue(name, value)
{
   ...
   _InternalErrorCode = API_CALL_PASSED_TO_LMS;
   setValueReturn = api.SetValue(name, value);
...
{//alert('sdfs');
   ...
   if ( apiHandle == null )
   {
      _log("Unable to locate the LMS's API Implementation.");
   }
   return apiHandle;
}
...
function findAPI(win)
{
   while ( (win.API_1484_11 == null)&&(win.parent != null)&&(win.parent != win) )
   {
      ...
      if ( findAPITries > 500 )
      {
         _log("Error finding API -- too deeply nested. 1.3");
         return null;
      }
      win = win.parent;
   }
...
function getAPI()
{
   ...
   if( theAPI ==null){
     ...
     if ( theAPI == null )
     {
        _log("Unable to find an API adapter");
     }
  }
   return theAPI;
}
